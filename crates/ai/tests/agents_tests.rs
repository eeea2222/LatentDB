...
//! and dry-run + approval-gated action execution. Fully offline (mock provider).

use latentdb_ai::{action, AgentAction, AiEngine};
...
async fn member(k: &Kernel, admin: &AuthContext) -> AuthContext {
    k.create_user(admin, "m@acme.com", "M", "pw-member-1", &["member".into()]).await.unwrap();
...
async fn setup_invoices(k: &Kernel, ctx: &AuthContext) {
    let def = ObjectTypeDef {
        ...
        fields: vec![
            ...
            FieldDefinition::new("due_date", "Due", FieldType::Date),
    ...
    k.create_record(ctx, &NewRecord {
        object_type: "invoice".into(),
        data: obj(json!({"number":"INV-PAID","amount":30000,"status":"paid","due_date":"2020-01-01"})),
        workspace_id: None,
    }).await.unwrap();
}
...
async fn bi_agent_answers_revenue_at_risk_with_citations() {
    ...
    let ans = ai.agents().bi_answer(&k, &ctx, "Why is revenue at risk this month?").await.unwrap();
...
async fn retrieval_respects_field_level_permissions() {
    ...
    // Admin can see the restricted secret in grounding snippets.
    let admin_ans = ai.agents().ask(&k, &adm, "INV", &["invoice".into()]).await.unwrap();
    let admin_snippets: String = admin_ans.sources.iter().map(|d| d.snippet.clone()).collect();
    assert!(admin_snippets.contains("TOPSECRET"));

    // The member retrieves the same invoices but the restricted field is gone.
    let mem_ans = ai.agents().ask(&k, &mem, "INV", &["invoice".into()]).await.unwrap();
    assert!(!mem_ans.sources.is_empty(), "member can still retrieve permitted records");
    let mem_snippets: String = mem_ans.sources.iter().map(|d| d.snippet.clone()).collect();
    assert!(!mem_snippets.contains("TOPSECRET"), "AI retrieval must not leak restricted fields");
}
...
async fn ai_disabled_returns_feature_disabled() {
    ...
    let err = ai.agents().finance_cashflow_risk(&k, &ctx).await.unwrap_err();
...
async fn action_dry_run_does_not_mutate() {
    ...
    let list = k
        .list_records(&ctx, "invoice", &latentdb_contracts::RecordFilter::default())
        .await
        .unwrap();
...
async fn execution_is_approval_gated_and_audited() {
    ...
    let err = latentdb_ai::execute(&k, &ctx, &action, false).await.unwrap_err();
    ...
    let audit = k
        .audit_query(&ctx, &AuditQuery { action: Some("ai.action.execute".into()), ..Default::default() })
        .await
        .unwrap();
...
async fn execution_blocked_when_flag_disabled() {
    ...
    let err = latentdb_ai::execute(&k, &ctx, &action, true).await.unwrap_err();
    assert_eq!(err.code, ErrorCode::FeatureDisabled);
}

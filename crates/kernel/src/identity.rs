...
impl Kernel {
    ...
    pub async fn list_users(&self, ctx: &AuthContext) -> latentdb_contracts::Result<Vec<User>> {
        self.authorize(ctx, Action::Read, "admin:users", None)
            .await?;
    ...
    pub async fn assign_role(
        ...
        self.authorize(ctx, Action::Configure, "admin:roles", None)
            .await?;
        let mut tx = self.pool().begin().await.map_err(map_db_err)?;
    ...
    pub async fn list_roles(&self, ctx: &AuthContext) -> latentdb_contracts::Result<Vec<Role>> {
        self.authorize(ctx, Action::Read, "admin:roles", None)
            .await?;

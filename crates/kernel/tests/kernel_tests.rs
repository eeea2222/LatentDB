...
async fn bootstrap_admin(k: &Kernel, slug: &str) -> latentdb_contracts::AuthContext {
    k.bootstrap_tenant(
        &format!("TestCo {slug}"),
        slug,
        "admin@example.test",
    ...
    let def = ObjectTypeDef {
        ...
        fields: vec![
            ...
            FieldDefinition::new("amount", "Amount", FieldType::Money)
                .required()
                .display(),
            FieldDefinition::new("status", "Status", FieldType::Enum).options(&[
                "draft",
                "submitted",
                "paid",
            ]),
...
async fn bootstrap_login_and_get_tenant() {
    ...
    let ctx = bootstrap_admin(&k, "testco").await;
    assert_eq!(ctx.role_keys, vec!["tenant_admin".to_string()]);
    assert!(!ctx.is_platform_admin);
    let tenant = k.get_tenant(&ctx).await.unwrap();
    assert_eq!(tenant.slug, "testco");
}

#[tokio::test]
async fn login_rejects_bad_password() {
    let k = Kernel::in_memory().await.unwrap();
    bootstrap_admin(&k, "testco").await;
    let err = k
        .login("testco", "admin@example.test", "wrong", "r", Source::Api)
        .await
        .unwrap_err();
...
async fn object_crud_emits_audit() {
    ...
    let updated = k
        .update_record(
            ...
            &rec.id,
            &RecordPatch {
                data: obj(json!({"status": "submitted"})),
            },
        )
        .await
        .expect("update");
    ...
    assert!(
        actions.contains(&"record.create"),
        "missing create audit: {actions:?}"
    );
...
async fn tenant_isolation_blocks_cross_tenant_access() {
    ...
    let list_b = k
        .list_records(&ctx_b, "invoice", &RecordFilter::default())
        .await
        .unwrap();
...
async fn member_cannot_configure_object_types() {
    let k = Kernel::in_memory().await.unwrap();
    let admin = bootstrap_admin(&k, "testco").await;
    ...
    let login = k
        .login("testco", "m@example.test", "pw-member-1", "r", Source::Api)
        .await
        .unwrap();
    let mctx = k
        .authenticate(&login.token, "r2", Source::Api)
        .await
        .unwrap();
    ...
    let denials = k
        .audit_query(
            &admin,
            &AuditQuery {
                action: Some("permission.denied".into()),
                ..Default::default()
            },
        )
        .await
...
async fn field_level_permission_hides_and_blocks_restricted_field() {
    ...
    assert!(
        seen.data.get("salary").is_none(),
        "member must not see restricted salary"
    );
...
async fn record_level_own_scope_blocks_others() {
    ...
    // Two members.
    for (email, _) in [("a@example.test", 0), ("b@example.test", 1)] {
        k.create_user(&admin, email, "U", "pw-member-1", &["member".into()])
            .await
            .unwrap();
    }
    let la = k
        .login("testco", "a@example.test", "pw-member-1", "r", Source::Api)
        .await
        .unwrap();
    let actx = k.authenticate(&la.token, "r", Source::Api).await.unwrap();
    let lb = k
        .login("testco", "b@example.test", "pw-member-1", "r", Source::Api)
        .await
        .unwrap();
...
async fn relations_link_records() {
    ...
    let inv = k
        .create_record(
            ...
            &NewRecord {
                object_type: "invoice".into(),
                data: obj(json!({"number": "R-1", "amount": 1, "status": "draft"})),
                workspace_id: None,
            },
        )
        .await
    ...
    let edge = k
        .relate(&admin, &inv.id, &cust.id, "billed_to")
        .await
        .unwrap();
...
async fn validation_rejects_bad_values() {
    ...
    let err = k
        .create_record(
            ...
            &NewRecord {
                object_type: "invoice".into(),
                data: obj(json!({"amount": 1, "status": "draft"})),
                workspace_id: None,
            },
        )
        .await
    ...
    let err = k
        .create_record(
            ...
            &NewRecord {
                object_type: "invoice".into(),
                data: obj(json!({"number": "x", "amount": 1, "status": "nope"})),
                workspace_id: None,
            },
        )
        .await
    ...
    let err = k
        .create_record(
            ...
            &NewRecord {
                object_type: "invoice".into(),
                data: obj(json!({"number": "x", "amount": 1, "status": "draft", "ghost": 1})),
                workspace_id: None,
            },
        )
        .await

...
fn transition(key: &str, from: &str, to: &str, requires_approval: bool) -> Transition {
    Transition {
        ...
        requires_approval,
        approval_policy: if requires_approval {
            Some("finance".into())
        } else {
            None
        },
...
async fn simple_transition_moves_state() {
    ...
    let res = k
        .transition_record(&admin, &id, "submit", None)
        .await
        .unwrap();
...
async fn invalid_transition_is_rejected() {
    ...
    let err = k
        .transition_record(&admin, &id, "approve", None)
        .await
        .unwrap_err();
...
async fn approval_gated_transition_waits_then_applies() {
    ...
    define_invoice_with_workflow(&k, &admin).await;
    let id = new_invoice(&k, &admin, "INV-1").await;
    k.transition_record(&admin, &id, "submit", None)
        .await
        .unwrap();
    ...
    let rec = k.get_record(&admin, &id).await.unwrap();
    assert_eq!(
        rec.workflow_state.as_deref(),
        Some("submitted"),
        "must not move before approval"
    );
    ...
    k.decide_approval(&admin, &approval_id, true, Some("ok"))
        .await
        .unwrap();
    let rec = k.get_record(&admin, &id).await.unwrap();
    ...
    let audit = k
        .audit_query(
            &admin,
            &AuditQuery {
                target_record_id: Some(id.clone()),
                ..Default::default()
            },
        )
        .await
        .unwrap();
    let linked = audit.iter().find(|e| {
        e.action == "workflow.transition" && e.approval_id.as_deref() == Some(approval_id.as_str())
    });
    assert!(
        linked.is_some(),
        "approved transition must be audited with approval id"
    );
...
async fn rejected_approval_does_not_move_record() {
    ...
    k.decide_approval(&admin, &approval_id, false, Some("over budget"))
        .await
        .unwrap();
    let rec = k.get_record(&admin, &id).await.unwrap();
    assert_eq!(
...
async fn member_cannot_transition_without_grant() {
    ...
    let err = k
        .transition_record(&mctx, &id, "submit", None)
        .await
        .unwrap_err();
